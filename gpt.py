...
class GPTLanguageModel(nn.Module):
    ...
    def __init__(self, vocab_size, n_embd, block_size, n_head, n_layer, device):
        ...
        # GPT-style weight initialization (same as Karpathy)
        self.apply(self._init_weights)

    def _init_weights(self, module):
        """GPT-style initialization keeps the network near an identity function at the start,
        so signals and gradients stay stable while the model learns corrections instead of
        exploding or dying.

        - Small, zero-mean weights (std = 0.02) → stable attention, non-saturated softmax
        - Zero bias → clean residual paths
        - Result → smooth training in deep Transformer stacks"""
